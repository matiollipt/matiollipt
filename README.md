# Cleverson Carlos Matiolli, Ph.D.

**Data Scientist · AI/ML Engineer · Computational Biologist**  
Paulínia, SP, Brazil &nbsp;|&nbsp; matiolli@gmail.com &nbsp;|&nbsp; [LinkedIn](https://www.linkedin.com/in/cleversonmatiolli/)

---

I build data products at the intersection of **life sciences, machine learning, and software engineering** — from RNA-seq pipelines and graph neural networks to LLM-enabled applications and geospatial climate tools.

---

## Current Work

**Independent Data Scientist & AI/ML Engineer** *(2024 – present)*

- **AidWeather / Agrilyzer** — Python client for NASA POWER climate data with ETL, agroclimatic feature engineering, and geospatial analytics for precision agriculture
- **AidGSEA** — R workflows for GEO RNA-seq studies: metadata validation, differential expression (DESeq2, edgeR, limma), and pathway analysis (GSEA)
- **Prot2GO / graphletGO** — GNN-based protein function prediction using PPI networks, sequence embeddings, and Gene Ontology labels (PyTorch Geometric)
- **SynViz / AidViz** — Real-time FFT audio visualiser (Electron + TypeScript) and reusable scientific visualisation components (Python)

---

## Skills

| | |
|---|---|
| **Languages** | Python, R, SQL, Bash, TypeScript/JavaScript |
| **ML & Deep Learning** | scikit-learn, PyTorch, PyTorch Geometric, TensorFlow/Keras, Hugging Face |
| **LLMs & Agents** | LangChain, LangGraph, LiteLLM, Ollama, RAG, MCP |
| **Bioinformatics** | RNA-seq, DESeq2, edgeR, limma, GO/GSEA, PPI networks, multi-omics |
| **Geospatial** | NASA POWER, xarray, GeoPandas, rasterio |
| **Tooling** | Git, Docker, uv, Electron, Jupyter, modular package design |

---

## Selected Publications

> 15+ peer-reviewed publications. Full list on [Google Scholar](https://scholar.google.com) and [ResearchGate](https://www.researchgate.net) — search **Cleverson C. Matiolli**.

| Year | Title | Journal |
|:---:|---|---|
| 2025 | Regulation of abscisic acid receptor mRNA stability: involvement of microRNA5628 in PYL6 transcript decay. Vieira JGP, Duarte GT, Barrera-Rojas CH, **Matiolli CC** et al. | *Plant Physiology* |
| 2024 | SUMOylation of rice DELLA protein SLR1 modulates transcriptional responses and improves yield under salt stress. **Matiolli CC** et al. | *Planta* |
| 2022 | Turning the Knobs: The Impact of Post-translational Modifications on Carbon Metabolism. **Matiolli CC** et al. | *Frontiers in Plant Science* |
| 2021 | Carbon/nitrogen metabolism and stress response networks — calcium-dependent protein kinases as the missing link? Alves HLS\*, **Matiolli CC**\* et al. (\*equal contribution) | *Journal of Experimental Botany* |
| 2021 | The sugar-responsive circadian clock regulator bZIP63 modulates plant growth. Viana AJC, **Matiolli CC** et al. | *New Phytologist* |
| 2020 | Novel molecular components involved in callose-mediated Arabidopsis defense against *Salmonella enterica* and *E. coli* O157:H7. Oblessuc PR, **Matiolli CC**, Melotto M. | *BMC Plant Biology* |
| 2018 | Circadian Entrainment in Arabidopsis by the Sugar-Responsive Transcription Factor bZIP63. Frank A, **Matiolli CC**, Viana AJC et al. | *Current Biology* |
| 2011 | The Arabidopsis bZIP63 transcription factor is a sensitive integrator of transient ABA and glucose signals. **Matiolli CC** et al. | *Plant Physiology* |

---

Open to remote and international opportunities in **Data Science · AI/ML · Computational Biology · Bioinformatics · Precision Agriculture**
